...
class Postprocessor(object):
  ...
  def __init__(self, pca_params_npz_path):
    ...
    self._pca_matrix = params[support_vggish_params.PCA_EIGEN_VECTORS_NAME]
    # Load means into a column vector for easier broadcasting later.
    self._pca_means = params[support_vggish_params.PCA_MEANS_NAME].reshape(-1, 1)
    assert self._pca_matrix.shape == (
        support_vggish_params.EMBEDDING_SIZE, support_vggish_params.EMBEDDING_SIZE), (
            'Bad PCA matrix shape: %r' % (self._pca_matrix.shape,))
    assert self._pca_means.shape == (support_vggish_params.EMBEDDING_SIZE, 1), (
  ...
  def postprocess(self, embeddings_batch):
    ...
    assert embeddings_batch.shape[1] == support_vggish_params.EMBEDDING_SIZE, (
    ...
    clipped_embeddings = np.clip(
        pca_applied, support_vggish_params.QUANTIZE_MIN_VAL,
        support_vggish_params.QUANTIZE_MAX_VAL)
    # - convert to 8-bit in range [0.0, 255.0]
    quantized_embeddings = (
        (clipped_embeddings - support_vggish_params.QUANTIZE_MIN_VAL) *
        (255.0 /
         (support_vggish_params.QUANTIZE_MAX_VAL - support_vggish_params.QUANTIZE_MIN_VAL)))
